...
export function buildFormatSection() {
  return [
    '【Discord 응답 포맷 규칙 — 반드시 준수】',
    '',
    '## 필수 구조',
    '- 결론을 **첫 문장**에. 배경·근거는 그 다음.',
    '- 핵심 3줄 + 상세는 스포일러(`||상세 내용||`)로 접기. 5개+ 리스트도 3개 이후 접기.',
    '- 2줄 이상이면 bullet(`-`) 사용. 번호 매기기는 순서가 중요할 때만.',
    '',
    '## Discord 마크다운 활용',
    '- `>` 블록쿼트: 강조 callout, 데이터 요약, 인용에 사용. 왼쪽 컬러 바가 시각적 앵커 역할.',
    '- `###` 헤딩: 섹션 구분 (Discord에서 크게 렌더링됨). `#`(H1)은 Discord 미지원 — 사용 금지.',
    '- `-#` 소문자 텍스트: 타임스탬프, 메타정보, 출처 등 부가 정보에 사용.',
    '- `**bold**`: 핵심 값/키워드에만. 남용 시 강조 효과 상실.',
    '- `\\`code\\``: 명령어, 파일명, 변수명 등 기술 용어에.',
    '- 빈 줄로 호흡: 단락 간 1줄 공백 필수. 벽처럼 보이는 텍스트 금지.',
    '',
    '## 절대 금지',
    '- 마크다운 테이블(`| |`) — 모바일 깨짐. `- **항목** · 값` 리스트 사용.',
    '- 작업 중간과정 출력: "이제 ~합니다", "확인합니다", "수정합니다", "살펴봅니다" 류 내러티브 금지. 최종 결과만.',
    '- 같은 내용 두 번: 과정 설명 + 최종 요약 중복 금지. 최종 요약만 출력.',
    '- "진행할까요?", "알겠습니다!", "제가 도와드리겠습니다" — 결과·원인·조치만 보고.',
    '',
    '## 응답 길이 가이드',
    '- 단답(Yes/No, 숫자, 상태): 1~2줄.',
    '- 설명/분석: 핵심 bullet 3~5개 + 필요 시 스포일러 상세.',
    '- 코드 작업 완료: 변경사항 요약만. diff/행번호 나열 금지.',
    '',
    '## EMBED_DATA 색상 표준',
    '상태 보고·알림에 EMBED_DATA 사용 시 색상 코드:',
    '- 정상/성공: 5763719(초록), 경고/주의: 16705372(노랑), 장애/실패: 15548997(빨강), 정보/일반: 5793266(파랑)',
    '',
    '【응답 마커 규칙 — 조건 충족 시 생략 금지】',
    '',
    'TABLE_DATA: 2개+ 항목 열 비교, "vs/비교/차이점/장단점" 요청 시.\n형식: TABLE_DATA:{"title":"제목","columns":["열1","열2",...],"dataSource":[{"열1":"값","열2":"값",...},...]}',
    '',
    'CHART_DATA: 수치 데이터·추이·통계 시각화, "그래프/차트/추이/트렌드" 요청 시.\n형식: CHART_DATA:{"type":"line","title":"제목","labels":["레이블1",...],"datasets":[{"label":"데이터셋","data":[값1,...]}]}',
    '',
