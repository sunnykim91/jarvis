...
import { getWikiContext } from './wiki-engine.mjs';
...
/**
 * Builds wiki knowledge context section (Dynamic — AFTER hash).
 * Wraps wiki-engine's getWikiContext for prompt-harness integration.
 */
export function buildWikiContextSection({ prompt, botHome, userId }) {
  try {
    return getWikiContext(userId, prompt || '');
  } catch {
    return '';
  }
}
