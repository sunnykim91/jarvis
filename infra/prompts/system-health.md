...
5) 크론 실패 감지: grep -cE 'ABORTED|FAILED' ~/jarvis/runtime/logs/cron.log 로 전체 카운트, tail -200 ~/jarvis/runtime/logs/cron.log | grep -E 'ABORTED|FAILED' 로 최근 실패 목록
...
필수: 끝에 bash ~/jarvis/runtime/scripts/health-check.sh 를 실행하여 health.json 갱신

🚫 RAG 절대 조작 금지 (위반 시 데이터 손실):
- ~/jarvis/runtime/state/rag-rebuilding.json 이 있으면 재인덱싱 진행 중. 절대 건드리지 말 것.
