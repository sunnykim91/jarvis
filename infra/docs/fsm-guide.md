...
> 최종 업데이트: 2026-04-14 (구현 U 추가: fsm-summary 평문 markdown 전환)
...
```bash
# 전체 태스크 상태 요약 (출력: 평문 markdown — reportFormat 섹션 모드)
# 🔴/🟡/🟢 이모지 + ### 헤딩으로 최근 실패/완료 섹션 구분
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs fsm-summary

# 전체 목록 (JSON)
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs list | \
  python3 -c "import json,sys; [print(t['id'], t['status'], t.get('retries',0)) for t in json.load(sys.stdin)]"

# 특정 태스크 상세
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs get council-insight
...
# 그룹 태스크 집계 (부모-자식 진행률)
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs list-with-aggregation | \
  python3 -c "
import json,sys
...
# failed → queued (수동 복구)
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs \
...
# 1. CB 파일 삭제
rm -f ~/jarvis/runtime/state/circuit-breaker/${TASK_ID}.json
...
# 3. 확인
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs cb-status "$TASK_ID"
```
...
/bin/bash ~/jarvis/runtime/bin/bot-cron.sh council-insight
```
...
ls ~/jarvis/runtime/state/active-tasks/
rmdir ~/jarvis/runtime/state/active-tasks/<TASK_ID>.lock 2>/dev/null || true
...
```bash
cat ~/jarvis/runtime/state/circuit-breaker/<TASK_ID>.json
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs cb-status <TASK_ID>
```
...
grep "CB cooldown expired" ~/jarvis/runtime/logs/stale-task-watcher.log

# 쿨다운 만료까지 남은 시간 계산 (수동)
python3 -c "
import json, time
with open('~/jarvis/runtime/state/circuit-breaker/<TASK_ID>.json') as f:
    cb = json.load(f)
...
const db=new DatabaseSync('~/jarvis/runtime/state/tasks.db');
...
# cp ~/jarvis/runtime/state/tasks.db ~/jarvis/runtime/state/tasks.db.bak.$(date +%s)
# rm ~/jarvis/runtime/state/tasks.db
```
...
- `script` 필드는 반드시 절대경로(`~/jarvis/runtime/...`) 사용 — 상대경로는 cron CWD 의존으로 ENOENT 발생
- `mcpConfig` 필드 (선택): `"serena"` 설정 시 `config/serena-mcp.json`이 MCP config로 사용됨 — 코드 탐색 태스크에 Serena LSP 싱글톤(port 24285)을 공유. `bot-cron.sh`가 `JARVIS_MCP_CONFIG` 환경변수로 export → `ask-claude.sh`가 `--mcp-config`에 전달. 미설정 시 기본값 `config/empty-mcp.json` (MCP 없음)
- `skipDuringRagRebuild: true` — `~/jarvis/runtime/state/rag-rebuilding.json` 존재 시 태스크 실행 보류 (RAG 재인덱싱 중 Claude 에이전트가 DB를 파괴하는 사고 방지). `system-health`에 적용됨.
...
```bash
# depends 충족 여부 확인
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs check-deps morning-standup

# 의존 태스크들 상태 직접 확인
node --experimental-sqlite --no-warnings ~/jarvis/runtime/lib/task-store.mjs list | python3 -c "
import json,sys
tasks = json.load(sys.stdin)
...
```bash
cat ~/jarvis/runtime/state/health.json | python3 -c "
import json,sys
...
| **Q** session-sync best-effort 전환 | `session-sync.sh`에서 `set -e` 제거 → `set -uo pipefail`만 유지. best-effort 스크립트(context-bus 갱신 실패는 치명적 오류 아님)에 `-e`를 적용하면 예측 불가 조건에서 exit 1이 발생해 CB를 누적시키는 문제 근본 수정. 개별 실패는 기존 `\|\| true` / 명시적 분기로 처리 | `scripts/session-sync.sh` |
| **R** auto-diagnose 메시지 개선 | 기존: `session-sync — FAILED (exit: 1)` (맥락 없음). 개선: tasks.json `description` 조회 → 태스크 역할 설명 + 실패 사유 + 로그 경로 안내. 동일 태스크 중복 제거(set 기반). python3 위임으로 bash 버전 무관하게 동작 | `scripts/auto-diagnose.sh` |
...
| **T** Serena MCP 싱글톤 공유 | `bot-cron.sh`가 tasks.json `mcpConfig` 필드를 읽어 `JARVIS_MCP_CONFIG` 환경변수로 export. `ask-claude.sh`가 `--mcp-config "${JARVIS_MCP_CONFIG:-empty-mcp.json}"` 사용. `mcpConfig: "serena"` → 상시 가동 Serena LSP 서버(port 24285)에 SSE 연결, `find_symbol`/`get_symbols_overview` 등 사용 가능. 코드 탐색 LLM 태스크(doc-sync-auditor, cron-auditor, security-scan) 3개에만 선별 적용. 비코드 79개는 기존 empty로 유지. ⚠️ 현재 3개 태스크 프롬프트에 Serena 도구 호출 지시 없음 → 효과 미실현. 프롬프트 수정 후 token-ledger로 before/after 측정 필요 | `bin/bot-cron.sh`, `bin/ask-claude.sh`, `config/serena-mcp.json` |
...
| **U** fsm-summary 출력 평문 markdown 전환 | `fsm-summary` CLI 출력을 raw 문자열 연결에서 `reportFormat({sections:[...]})` 호출로 교체. 기존: `최근 실패:\n  - \`id\`` 단순 문자열 (영역 구분 없음). 변경 후: `### ❌ 최근 완료 (N)` / `### 🟢 최근 완료 (N)` markdown 헤딩으로 Discord에서 굵고 크게 렌더. 상태 요약은 `done N개 · pending N개 · ...` 한 줄. KST 푸터(`-# 2026-... · fsm-summary`) 자동 첨부. `formatters.js` import 추가 | `lib/task-store.mjs`, `discord/lib/formatters.js` |
...
5. `COMMIT`
6. `toStatus === 'done'` 시: RAG 피드백 파일 `~/jarvis/runtime/rag/task-outcomes-YYYY-MM.md`에 결과 append
7. 업데이트된 태스크 객체 반환
...
**동작 흐름**:
1. `~/jarvis/runtime/state/effective-tasks.json` 읽기 (없으면 `config/tasks.json` fallback)
...
```bash
# 현재 백로그 목록 확인
cat ~/jarvis/runtime/config/dev-backlog.json | jq '.backlog[] | {id, title, status, phase}'

# 특정 작업 상세
cat ~/jarvis/runtime/config/dev-backlog.json | jq '.backlog[] | select(.id=="dashboard-archive")'

# 차단 중인 작업 확인
cat ~/jarvis/runtime/config/dev-backlog.json | jq '.blocked_by'
```
...
1. **상태 변경** (작업 착수 시):
   ...
   jq '.backlog[] |= if .id=="dashboard-archive" then .status="in-progress" else . end' \
     ~/jarvis/runtime/config/dev-backlog.json > /tmp/dev-backlog.tmp && \
     mv /tmp/dev-backlog.tmp ~/jarvis/runtime/config/dev-backlog.json
   ```
