...
| **E** enqueue `--post-title` 지원 | `enqueue` CLI에 `--post-title <제목>` 플래그 추가. 값은 `meta.postTitle`에 저장. `dev-task-poller.sh`가 board 소스 포스트 제목을 태스크 메타에 포함하여 board UI 및 알림에서 출처 토론 제목 표시 가능 | `lib/task-store.mjs`, `scripts/dev-task-poller.sh` |
...
| **J** enqueue `--timeout` 플래그 | `task-store.mjs enqueue`에 `--timeout <초>` 옵션 추가. 값은 `meta.timeout`에 저장되며 `jarvis-coder.sh`가 claude -p 실행 시 사용. `dev-task-poller.sh`는 board dispatch 태스크 enqueue 시 `--timeout 600` 전달 (기존 기본값 300s에서 증가 — 개발 태스크가 stale-watcher 10분 기준에 걸리던 문제 해소) | `lib/task-store.mjs`, `scripts/dev-task-poller.sh` |
...
| **Q** jarvis-coder 이벤트 트리거 전환 | 2시간 크론 → `dev.task.queued` 이벤트 트리거(60초 debounce). `task-store.mjs enqueue` 시 `emit-event.sh` 자동 발행 (SSoT). 최대 지연 2시간 → 30초. dev-task-poller 직접 호출 제거, crontab */2 제거, tasks.json 22:55 daily fallback 유지 | `lib/task-store.mjs`, `config/tasks.json`, `scripts/dev-task-poller.sh` |
| **Q** board dispatch 태스크 maxBudget 전달 | `dev-task-poller.sh` enqueue 시 `--maxBudget` 미전달로 기본값 $1.00 적용 → 복잡한 코딩 태스크(344s 실행)가 budget 초과 실패. `task-store.mjs enqueue`에 `--maxBudget` 플래그 추가, `dev-task-poller.sh`에 우선순위별 budget 계산: urgent=$5.00, high=$3.00, medium=$2.00, low=$1.50 | `lib/task-store.mjs`, `scripts/dev-task-poller.sh` |
