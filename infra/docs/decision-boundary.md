...
### 1-2. 로컬 상태 쓰기
- `~/jarvis/runtime/state/` 파일 생성/수정 (context-bus.md, health.json 등)
- `~/jarvis/runtime/context/` 컨텍스트 파일 갱신
- `~/jarvis/runtime/logs/` 로그 기록
...
### 2-2. 코드 / 설정 변경 (HIGH)
- `~/jarvis/runtime/config/tasks.json` 수정 (크론 스케줄 변경)
...
### 2-5. 예산 초과 판단 (HIGH)
- `~/jarvis/runtime/config/team-budget.json` 초과 예상 시
...
위치: `~/jarvis/runtime/state/decisions/YYYY-MM-DD.jsonl`
...
관련 문서:
- `~/jarvis/runtime/config/monitoring.json` — 자동 발송 임계값
- `~/jarvis/runtime/config/team-budget.json` — 팀별 예산 한도
- `~/jarvis/runtime/adr/ADR-INDEX.md` — 아키텍처 결정 기록
