...
set -euo pipefail
LOG=~/.jarvis/logs/policy-fix-verify.log
echo "[verify] $(date '+%Y-%m-%d %H:%M:%S KST')" >> "$LOG"

# 새 매니페스트 생성
node ~/.jarvis/scripts/policy-fix-classify.mjs >> "$LOG" 2>&1

# C-1이었던 28개의 새 카테고리 추출 (어떻게 변했는지)
PRE=/tmp/c1-labels.txt
RESULT=$(python3 -c "
import csv
with open('$HOME/.jarvis/state/policy-fix-manifest.csv') as f:
...
# Discord 알림
node ~/.jarvis/scripts/discord-send-msg.mjs "jarvis-system" "## 🔍 Phase 1 24h 검증 결과

\`\`\`
$RESULT
\`\`\`

세부: \`tail -50 ~/.jarvis/logs/policy-fix-verify.log\`" 2>/dev/null || true
